    ...
    <div slot="banner" class="sliceBanner">
      <div class="sliceBannerTitle">
        ...
        <div class="sliceBannerTitleRight">
          ...
          <el-select  v-model="hospitalValue" placeholder="医院" @change="handleHospitalChange" class="select-option">
            <el-option
          ...
          <div style="position:relative" class="progress-button">
            <el-button size="mini"
                       type="primary"
                       @click="scanSlice"
                        :disabled="isShowButton">扫描切片
            </el-button>
            <div class="entry-progress" v-if="isShowProgress" style="position:absolute;width:100%;top:34px;">
              <div class="align-self-end" >
                <el-progress :percentage="progressValue" :text-inside="true" :stroke-width="15"></el-progress>
              </div>
            </div>
            <div v-if="isShowDetail" style="position:absolute;left:38%;">
              <el-popover
                placement="top-start"
                width="150"
                trigger="hover"
                popper-class="detail-popover">
                <p >一共扫描了{{allSlice}}张切片，其中有{{outSlice}}张是外来数据</p>
  ...
  export default {
    ...
    async created() {
     await this.getHospitalSelect();
      this.queryList()
      this.subscribe(JSON.parse(localStorage.userInfo).username,'scan');
      this.$http.get(`/slice/queue_status`,{params:{queue:`${JSON.parse(localStorage.userInfo).username}`}}).then((res)=>{
        if(res.body.is_running===0){
          this.isShowProgress=false;
          this.isShowDetail=false;
        }
        else if(res.body.is_running===1){
          this.isShowProgress=true;
          this.isShowButton=true;
          this.subscribe(JSON.parse(localStorage.userInfo).username,'scan');
          this.$message.success('正在切片扫描中...');
        }
      })
    },
    mixins: [hzztStomp],
    methods: {
      ...
      getHospitalSelect(){
        this.getCategories();
        this.$http.get('/source').then(({body})=>{
          if(body.ret_code===0){
      ...
      handleHospitalChange(val){
        if(val===''){
          ...
          let sliceOptions = this.hospitalMessages.find((item)=>item.id===val);
          if(sliceOptions!==undefined){
            sliceOptions = sliceOptions.cates||[];
      ...
      scanSlice() {
        this.isShowDetail=false;
        let params={
          queue:`${JSON.parse(localStorage.userInfo).username}`
        }
        Api.scanSlice(params).then((res)=>{
          if(res.body.ret_code===0){
            ...
            this.$message.error(res.body.ret_msg||'请先配置目录');
          }
        })
