  ...
  <Pannel v-bind:title="titleList" v-if="labelMain" :bottom="false">
    <div slot="banner" class="side">
      ...
      <div class="checkright">
        ...
        <span style="margin-right:10px;line-height: 30px">
          <el-select v-model="hospitalValue"
                     placeholder="医院"
        ...
        <span style="margin-right:10px;line-height: 30px">
          <el-select v-model="userValue" placeholder="审核人" @change="handleUserChange" class="select-option">
          <el-option
        ...
        <div class="analyze-div">
          <el-dropdown split-button type="primary" class="top-allot" @command="handleAnalyzeCommand">
            解析
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="autoAnalyze" :disabled="isShowAllotProgress">自动解析</el-dropdown-item>
              <el-dropdown-item  command="someAnalyze" :disabled="isShowAllotProgress">批量解析</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          <div class="entry-progress" v-if="isShowAllotProgress" >
            <div class="align-self-end">
              <el-progress :percentage="progressAllotValue" :text-inside="true" :stroke-width="15" ></el-progress>
              <span >{{currentAllotCount}}/{{totalAllotCount}}</span>
            </div>
          </div>
        </div>
    ...
    <div slot="main" class="checkmain">
      <el-table ref="multipleTable" :data="tableData"
                ...
                width="100%" height="100%" v-loading="loading">
        ...
        <el-table-column prop="updated_at" label="审核分配时间" min-width="180">
        </el-table-column>
        <el-table-column label="操作分配"
                         width="225px"
                         fixed="right">
          <template slot-scope="scope">
            <el-button size="mini" @click="allot(scope.row)" type="text" class="reallocateBtn">
              重新分配
            </el-button>
            ...
            <el-button size="mini" @click="downloadJSON(scope.row)" type="text" class="reallocateBtn"
                       v-if="scope.row.label_count>0">
              JSON下载
            </el-button>
            <el-button size="mini" :disabled="isShowAllotProgress" @click="analyzeSlice(scope.row)" type="text" class="reallocateBtn" v-if="scope.row.is_mask==1">
              解析
            </el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        categoryMessages:[],
        isMask:0,
        isShowAllotProgress:false,
        progressAllotValue:60,
        currentAllotCount:0,
        totalAllotCount:0,
      }
    },
    ...
    async created() {
      ...
      this.queryList();
      const queue = `allocated_label_parse_json_${JSON.parse(localStorage.userInfo).username}`;
      this.subscribe(queue,'AllocatedAnalyze');
    },
    methods: {
      //获取医院下拉框
      getHospitalSelect(){
        ...
        this.$http.post('/analytical/batch',params).then(({body})=>{
          if(body){
            if(body.ret_code===0){
              ...
              this.$message.success(body.ret_msg||'切片解析失败');
            }
          }
